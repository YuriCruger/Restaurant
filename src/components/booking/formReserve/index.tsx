...
const schema = z.object({
    name: z.string().min(1, 'Name is empty').transform((value) => {
...
export default function index({ setReserveData, setPeople, setModalOpen, modalOpen }: Props) {
    ...
    const Increment = () => {
        ...
        if (!modalOpen) {
            setInitialPeople((prevPeople) => (prevPeople < 10 ? prevPeople + 1 : prevPeople));
        }
    };
    ...
    const Decrement = () => {
        ...
        if (!modalOpen) {
            setInitialPeople((prevPeople) => (prevPeople > 1 ? prevPeople - 1 : prevPeople));
        }
    };

    const { register, handleSubmit, formState: { errors, isSubmitting }, setFocus, reset, } = useForm<FormProps>({
    ...
    return (
        <S.Container>
            ...
            <S.Form onSubmit={handleSubmit(onSubmit)} >
                ...
                <S.Counter>
                    <S.CounterManipulator type='button' onClick={Decrement}>-</S.CounterManipulator>
                    <strong><span>{initialPeople}</span> people</strong>
                    <S.CounterManipulator type='button' onClick={Increment}>+</S.CounterManipulator>
                </S.Counter>

                <S.Button type='submit' disabled={isSubmitting}>{isSubmitting ? 'Sending...' : 'MAKE A RESERVATION'}</S.Button>
            </S.Form>
        </S.Container>
    )
}
